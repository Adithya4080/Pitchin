...
import { Loader2, Upload, X, Plus, Building2, ImageIcon, ArrowLeft, HelpCircle, Pencil, Trash2, Link2, Users } from 'lucide-react';
...
export default function ProviderDashboard() {
  ...
  const toggleSubCategory = (id: number) => {
    ...
    set('sub_categories', current.includes(id) ? current.filter((x) => x !== id) : [...current, id]);
  };
    ...
    <div className="min-h-screen bg-muted/30 pb-24">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        ...
        <Card className="overflow-hidden">
          <div
            className="h-36 bg-gradient-to-r from-primary/20 to-primary/5 bg-cover bg-center relative flex items-end justify-end p-3"
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Company basics</CardTitle>
            <CardDescription>Who you are and what you do</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-1.5">
              <Label>Company / brand name</Label>
              <Input value={form.name || ''} onChange={(e) => set('name', e.target.value)} placeholder="e.g. Acme Legal Co." />
            </div>
            <div className="space-y-1.5">
              <Label>Tagline</Label>
              <Input
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Category & services</CardTitle>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-1.5">
              ...
              <div className="flex gap-2">
                <Select value={form.category ? String(form.category) : undefined} onValueChange={(v) => { set('category', Number(v)); set('sub_categories', []); }}>
            ...
            {selectedCategory && (
              <div className="space-y-1.5">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Badge variant="outline" className="cursor-pointer border-dashed" onClick={() => setNewSubCategoryOpen(true)}>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            <div>
              <CardTitle className="text-base">Past work & collaborations</CardTitle>
              <CardDescription>
                {exists ? 'Showcase real projects — with a link founders can click through to' : 'Save your bio first, then add projects here'}
              </CardDescription>
            </div>
            {exists && (
              <Button size="sm" variant="outline" onClick={openNewProjectDialog}>
          ...
          <CardContent>
            {!exists ? (
              <div className="text-sm text-muted-foreground border rounded-lg p-4 text-center bg-muted/40">
                Publish your bio above to unlock portfolio uploads.
              </div>
            ) : !myProvider.media || myProvider.media.length === 0 ? (
              <div className="text-sm text-muted-foreground border rounded-lg p-4 text-center bg-muted/40 flex flex-col items-center gap-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                {myProvider.media.map((m) => (
                  <div key={m.id} className="group rounded-lg overflow-hidden border">
        ...
        <Card>
          ...
          <CardContent>
            {!exists ? (
              ...
              <div className="flex flex-wrap gap-2">
                {collaborators.map((c) => (
                  <div key={c.id} className="flex items-center gap-2 border rounded-lg pl-2 pr-1 py-1.5">
                    {c.logo_url ? (
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            ...
            {exists && (
              <Button size="sm" variant="outline" onClick={openNewFaqDialog}>
                <Plus className="h-3.5 w-3.5 mr-1.5" /> Add
              </Button>
            )}
          </CardHeader>
                ...
                {faqs.map((f) => (
                  <div key={f.id} className="border rounded-lg p-3 flex items-start justify-between gap-3">
                    ...
                    <div className="flex gap-1 shrink-0">
                      <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => openEditFaqDialog(f.id, f.question, f.answer)}>
                        <Pencil className="h-3.5 w-3.5" />
                      </Button>
                      <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => handleDeleteFaq(f.id)}>
